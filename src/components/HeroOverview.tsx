...
export const HeroOverview: React.FC<HeroOverviewProps> = ({
  ...
  return (
    <section id="hero-overview-section" className="space-y-4 pt-4">
      {/* Top Banner Stats Grid */}
      <div className="grid grid-cols-2 sm:grid-cols-3 lg:grid-cols-6 gap-3">
        {/* Metric 1: Total Scrobbles */}
        <div className={`p-4 rounded-2xl ${theme.cardBg} border ${theme.cardBorder} shadow-sm space-y-1 relative overflow-hidden group`}>
          ...
          <p className="text-[11px] text-zinc-500 truncate">
            ~{listeningStats.totalListeningHours} hours logged
          </p>
        </div>

        {/* Metric 2: Unique Artists */}
        <div className={`p-4 rounded-2xl ${theme.cardBg} border ${theme.cardBorder} shadow-sm space-y-1`}>
          <div className="flex items-center justify-between">
            <span className="text-[11px] font-bold text-zinc-400 uppercase tracking-wider">
              Artists
            </span>
        ...
        <div className={`p-4 rounded-2xl ${theme.cardBg} border ${theme.cardBorder} shadow-sm space-y-1`}>
          ...
          <p className="text-[11px] text-amber-400/80 truncate">
            {topTrack ? `${topTrack.artist} (${topTrack.playCount} plays)` : '—'}
          </p>
        </div>
        ...
        <button
          ...
          className={`p-4 rounded-2xl ${theme.cardBg} border ${theme.cardBorder} shadow-sm space-y-1 text-left hover:border-cyan-500/50 transition-all group cursor-pointer`}
        >
          <div className="flex items-center justify-between">
            <span className="text-[11px] font-bold text-zinc-400 uppercase tracking-wider">
              Genre Hot 5s
          ...
          <p className="text-[11px] text-cyan-400/80 truncate">
            View Genre Hot 5s &rarr;
          </p>
        </button>
        ...
        <button
          ...
          className={`p-4 rounded-2xl ${theme.cardBg} border ${theme.cardBorder} shadow-sm space-y-1 text-left hover:border-yellow-500/50 transition-all group cursor-pointer`}
        >
          <div className="flex items-center justify-between">
            <span className="text-[11px] font-bold text-zinc-400 uppercase tracking-wider">
              Plaques Forged
          ...
          <p className="text-[11px] text-yellow-400/80 truncate">
            View Plaque Wall &rarr;
          </p>
        </button>
        ...
        <button
          ...
          } shadow-sm space-y-1 text-left hover:border-indigo-500/50 transition-all cursor-pointer`}
        >
          <div className="flex items-center justify-between">
            <span className="text-[11px] font-bold text-zinc-400 uppercase tracking-wider">
              Remasters / Deluxes
          ...
          <div className="text-2xl font-black text-white tracking-tight font-mono">
            {unmergedCount} <span className="text-xs font-normal text-zinc-400">sets</span>
          </div>
          <p className="text-[11px] text-indigo-400 truncate">
            {unmergedCount > 0 ? 'Consolidate plays &rarr;' : 'Catalog clean'}
          </p>
        </button>
      </div>
    </section>
