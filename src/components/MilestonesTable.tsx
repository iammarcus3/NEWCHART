...
export const MilestonesTable: React.FC<MilestonesTableProps> = ({
  ...
  return (
    <div ref={tableTopRef} className="space-y-2.5">
      {/* Top Pagination & Record Count Toolbar */}
      <div className="flex flex-wrap items-center justify-between gap-2 px-3 py-2 rounded-xl bg-zinc-900/60 border border-zinc-800/80 text-xs">
        <div className="text-zinc-400 font-medium">
          Showing <span className="text-white font-bold">{startRecord}–{endRecord}</span> of <span className="text-white font-bold">{items.length}</span> records
        </div>
          ...
          {totalPages > 1 && (
            <div className="flex items-center gap-1">
              <button
                ...
                disabled={safePage <= 1}
                className="p-1 rounded-lg bg-zinc-800 hover:bg-zinc-700 disabled:opacity-40 disabled:hover:bg-zinc-800 text-zinc-300 transition-colors"
                title="First page"
              >
              ...
              <span className="px-2 text-zinc-300 font-bold font-mono">
                {safePage} / {totalPages}
              </span>
        ...
        <div className="flex items-center justify-between pt-2 px-1 text-xs text-zinc-400">
          ...
          <div className="flex items-center gap-1">
            <button
              onClick={() => handlePageChange(safePage - 1)}
              disabled={safePage <= 1}
              className="px-3 py-1 rounded-lg bg-zinc-800 hover:bg-zinc-700 disabled:opacity-40 text-zinc-200 transition-colors cursor-pointer"
            >
              Previous
            </button>
            <button
