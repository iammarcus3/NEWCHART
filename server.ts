...
import {
  resolveUndersizedAlbumsWithAI,
  generateHeuristicAlbumSuggestions,
} from './src/server/geminiAlbumResolver.ts';
...
async function startServer() {
  ...
  app.post('/api/ai/suggest-album-merges', async (req, res) => {
    ...
    try {
      ...
      console.warn('Notice: Using heuristic album suggestions fallback:', error?.message || error);
      const fallbackSuggestions = generateHeuristicAlbumSuggestions(candidates);
